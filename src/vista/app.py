...
class ConversorApp:
    def __init__(self, root):
        ...
        self.origen_combo = ttk.Combobox(self.frame, values=['USD', 'EUR', 'JPY','PEN'],
                                         width=10)
        ...
        self.destino_combo = ttk.Combobox(self.frame, values=['USD', 'EUR', 'JPY','PEN'],
                                          width=10)
        self.destino_combo.grid(row=2, column=1, sticky=tk.W)
        self.destino_combo.current(1)
        # Botón de conversión
        self.convertir_btn = ttk.Button(self.frame, text="Convertir",
